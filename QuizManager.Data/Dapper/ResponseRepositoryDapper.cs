    ...
    public class ResponseRepositoryDapper : IResponseRepository
    {
        ...
        public IEnumerable<ResponseItem> GetResponseItemsForQuiz(int quizId)
        {
            using (var conn = new SqlConnection(Settings.GetConnectionString()))
            {
                ...
                const string sql =
                    "SELECT r2.id AS id, quiz.id AS quizId, round, questionNumber, UserName AS [name], responseText, points, timestamp FROM quiz JOIN question q ON quiz.id = q.quizId JOIN response r2 ON q.id = r2.questionId join AspNetUsers ANU on r2.userId = ANU.Id WHERE quiz.id=@quizId ORDER BY timestamp";
